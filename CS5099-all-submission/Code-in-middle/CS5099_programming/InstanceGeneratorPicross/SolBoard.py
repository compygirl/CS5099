...
class SolutionBoard:
    ...
    def readImage(self):
        img = imread(os.path.join(self.workingFolder, self.filename), as_grey = True)
        return img
    ...
    def createSolutionBoard(self):
        ...
        mainRows, mainCols = self.getInstanceDimensions()
...
#         content += "letting mainRow be "  + str(len(solutionBoard)) + '\n'
#         content += "letting mainCol be "  + str(len(solutionBoard[0])) + '\n'
#
#         topMatrix = self.createTopMatrix(solutionBoard)
#         topRows = len(topMatrix)
#         content += "letting topRows be "  + str(topRows) + '\n'
#
#
#         leftMatrix = self.createLeftMatrix(solutionBoard)
#         leftCols = len(leftMatrix[0])
#         content += "letting leftCols be "  + str(leftCols) + '\n'
#
#         content +="letting leftMatrix be [ \n"
#         leftMatrixContent=self.transMatrixTofileFormat(leftMatrix)
#         content +=leftMatrixContent + "]\n"
#
#         content += "letting topMatrix be [ \n"
#         topMatrixContent = self.transMatrixTofileFormat(topMatrix)
#         content +=topMatrixContent + "] \n"
#         return content
#
#     def executeSavileRow(self, filename):
#         # call(["zsh", "-c", "'/Users/aigera/Desktop/Constraint Programming/savilerow-1.6.4-mac/savilerow' picross_solver.eprime "+filename+" -run-solver -num-solutions 5 -solver-options '-varorder conflict -prop-node SACBounds'"])
